function autocompletar() {
    const inputApeConsultant = document.querySelector('#ApeConsultant');
    let indexFocus = -1;

    inputApeConsultant.addEventListener('input', function () {
        const ApeConsultant = this.value;

        if (!ApeConsultant) return false;
        ...
        httpRequest('../dev/dev_php/Autocomplete-consultant_ape.php?ApeConsultant=' + ApeConsultant, function(){

            const arreglo = JSON.parse(this.response);

            //validar arreglo VS inputApeConsultant
            if (arreglo.length == 0) return false;
            arreglo.forEach(item => {
                if (item.substr(0, ApeConsultant.length) == ApeConsultant) {
                    const elementoLista = document.createElement('div');
                    elementoLista.innerHTML = `<strong>${item.substr(0, ApeConsultant.length)}</strong>${item.substr(ApeConsultant.length)}`;
                    elementoLista.addEventListener('click', function () {
                        inputApeConsultant.value = this.innerText;
                        cerrarLista();
                        return false
                    })
    ...
    inputApeConsultant.addEventListener("keydown", function (e) {
